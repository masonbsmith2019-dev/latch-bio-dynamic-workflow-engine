...
def square_batch(ctx: ExecutionContext, numbers: List[int]) -> int:
    ...
    ctx.promise(
        LimitedSpawns(square, max_count=len(numbers)-1),
...
if __name__ == "__main__":
    outdir = Path("outputs/map_reduce_spawn_budget_violation")
    ...
    print("Run complete → outputs/map_reduce_spawn_budget_violation (events.jsonl, dots/, pngs/)")
